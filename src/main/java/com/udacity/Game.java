...
public class Game {
    ...
    public String checkGameWinner(char[][] grid) {
        String result = "None";
        char[] whichSign = {'x', 'o'};
        for (int i = 0; i < whichSign.length; i++) {
            for (int j = 0; j < grid.length; j++) {
                if (grid[j][0] == whichSign[i] && grid[j][1] == whichSign[i] && grid[j][2] == whichSign[i]) {           // check rows
                    result = "wins: " + whichSign[i];
                } else if (grid[0][j] == whichSign[i] && grid[1][j] == whichSign[i] && grid[2][j] == whichSign[i]) {    // check columns
                    result = "wins: " + whichSign[i];
                } else if (grid[0][0] == whichSign[i] && grid[1][1] == whichSign[i] && grid[2][2] == whichSign[i]) {    // check 1st diagonal
                    result = "wins: " + whichSign[i];
                } else if (grid[0][2] == whichSign[i] && grid[1][1] == whichSign[i] && grid[2][0] == whichSign[i]) {    // check 2nd diagonal
                    result = "wins: " + whichSign[i];
                } else if ( freeSpots == 0){
                    result = "Tie";
                }
